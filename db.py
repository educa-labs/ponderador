...
class DB():
    ...
    def __init__(self):
        while True:
            try:
                self.conn = psql.connect("dbname=simulador user=fahaase password=123456asd host=localhost port=5432")
    ...
    def poblar(self):
        ...
        a = self.cur.execute("""
            CREATE TABLE simulaciones(id serial PRIMARY KEY, nombre text, rut text,
            mail text, region text, celular text,
            mat int, len int, cie int, his int, nem int, ran int, pond int,
            dif int, universidad text, carrera text, fecha timestamp);
            """)
    ...
    def simulation(self, form):
        ...
        dicc = {"nem":0, "ran":0, "mat":0, "len":0, "opt": 0, "pond":0, "corte":0, "dif": 0, "web": ""}

        self.cur.execute("""SELECT web FROM universities WHERE id = %s""",(uid,))
        for i in self.cur:
            dicc["web"] = i[0]
    ...
    def selectSimulaciones(self):
        self.cur.execute("SELECT * FROM simulaciones;")
        with open("consultas.csv", "w") as writer:
